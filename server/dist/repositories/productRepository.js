...
const productsRepository = {
    ...
    checkCountProduct: (db, id) => __awaiter(void 0, void 0, void 0, function* () {
        return yield db.collection("t-shirt1").findOne({
            _id: new mongodb_1.ObjectId(id)
        });
    }),
    // updateCountInProduct: async (db, productId,size, count) => {
    //   return await db.collection("t-shirt1").updateOne(
    //     {_id: new ObjectId(productId)},
    //     {"$inc": {"size.$[elem].count": -count}},
    //     [{"elem.size": size}]
    //   )
    // }
    // decreaseCountProduct: async (db, id, size)
};
